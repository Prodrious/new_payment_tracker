...
import { 
  ...
  RefreshCw, AlertCircle, WifiOff, CloudOff, Cloud
} from 'lucide-react';
...
export default function Home() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // We don't block the user, we just start with empty arrays
      setStudents([]);
      setSchedules([]);
    } finally {
      setIsLoading(false);
  ...
  const handleClearData = async () => {
    if (confirm("Clear all local and cloud data?")) {
      setStudents([]);
      setSchedules([]);
      try {
  ...
  if (isLoading) return (
    <div className="h-screen flex flex-col items-center justify-center bg-[#0f172a] text-white">
      ...
      <p className="text-[10px] font-black uppercase tracking-[0.4em] text-indigo-400">Initializing Engine...</p>
    </div>
  );
            ...
            <div className="flex items-center gap-4 bg-slate-800/30 p-4 rounded-[2rem] border border-slate-700/30">
              ...
              <div className="min-w-0">
                ...
                <p className={`text-[9px] font-black uppercase tracking-tighter ${cloudStatus === 'connected' ? 'text-emerald-400' : 'text-amber-400'}`}>
                  {cloudStatus === 'connected' ? 'Online Mode' : 'Local Mode'}
                </p>
              </div>
            </div>
          </div>
        ...
        <header className="h-24 bg-white/60 backdrop-blur-xl border-b border-slate-200/60 flex items-center justify-between px-10 shrink-0 z-30">
            ...
            <div className="flex items-center gap-4">
              ...
              {isSyncing ? (
                <div className="flex items-center gap-2 bg-indigo-50 px-3 py-1 rounded-full border border-indigo-100 animate-pulse">
                  ...
                  <span className="text-[10px] font-black text-indigo-500 uppercase tracking-widest">Syncing...</span>
                </div>
              ) : cloudStatus === 'connected' ? (
                <div className="flex items-center gap-2 bg-emerald-50 px-3 py-1 rounded-full border border-emerald-100">
                  <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" />
                  <span className="text-[10px] font-black text-emerald-500 uppercase tracking-widest">Cloud Active</span>
                </div>
              ) : (
                <div className="flex items-center gap-2 bg-amber-50 px-3 py-1 rounded-full border border-amber-100 group relative cursor-help">
                  ...
                  <span className="text-[10px] font-black text-amber-500 uppercase tracking-widest">Local Mode</span>
                  {lastError && (
                    <div className="absolute top-full left-0 mt-2 p-3 bg-slate-900 text-white text-[9px] font-mono rounded-xl w-64 opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-50">
                      Reason: {lastError}
                    </div>
                  )}
                </div>
          ...
          <div className="flex items-center gap-6">
            <div className="hidden md:flex flex-col items-end">
               <span className="text-[10px] font-black text-slate-900 uppercase tracking-widest">TutorTrack Pro</span>
               <span className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">V1.2.0-Production</span>
            </div>
