...
export default function Home() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setDbError(error.message || "Failed to connect to MongoDB.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  if (isLoading) return (
    <div className="h-screen flex flex-col items-center justify-center bg-[#0f172a] text-white">
      ...
      <p className="text-sm font-black uppercase tracking-[0.3em] opacity-50">Connecting to Next.js API...</p>
    </div>
  );
    ...
    <div className="h-screen flex items-center justify-center bg-[#f8fafc] p-6">
      <div className="max-w-md w-full bg-white rounded-[2.5rem] shadow-2xl border border-rose-100 p-10 text-center animate-slide-in">
        ...
        <div className="space-y-4">
          ...
          <a href="https://vercel.com/dashboard" target="_blank" rel="noopener noreferrer" className="w-full flex items-center justify-center gap-2 bg-slate-100 text-slate-600 py-4 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-slate-200 transition-all">
          ...
          <div className="mt-auto pt-6 border-t border-slate-800 space-y-4">
            ...
            <div className="flex items-center gap-3 px-2 bg-slate-800/40 p-3 rounded-2xl border border-slate-700/50">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[10px] text-slate-500 truncate uppercase font-bold">Next.js Cloud</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <header className="h-20 bg-white/80 backdrop-blur-md border-b border-slate-200 flex items-center justify-between px-8 shrink-0 z-30">
          ...
          <div className="flex items-center gap-4">
            <div className="hidden sm:flex flex-col items-end mr-4">
              <span className="text-[10px] font-black text-indigo-600 uppercase tracking-widest">Database: Online</span>
